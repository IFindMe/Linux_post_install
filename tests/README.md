...
| `t-share-webdav.sh` | `pos share webdav` (rclone): dispatch/deps/paths/flag/auth failures, DRY_RUN share/enable happy paths, addr precedence, TLS advisory matrix, masked status/list, webdav.env origin, ufw advisory, 644 unit + 600 env persistence, bus pre-flight orphan guard, idempotent unshare/disable, live spawn+list (restart/unshare-stop skipped — tool defect WEBDAV-01, see tester report 2026-09-18), sentinel-leak tripwire + fail-closed guards |